...
class TestFeedDao(unittest.TestCase):
    ...
    feed_info1 = ["title1", "link1", "source1", datetime.now().astimezone(pytz.timezone("Asia/Tokyo")), "summary1", "category1"]
    feed_info2 = ["title2", "link2", "source2", datetime.now().astimezone(pytz.timezone("Asia/Tokyo")), "summary2", "category2"]
    ...
    def test_get_latest_time(self):
        ...
        self.dao.add_feed(Feed(self.feed_info2[0], self.feed_info2[1], self.feed_info2[2], self.feed_info2[3], self.feed_info2[4], self.feed_info2[5]))
        self.assertEqual(self.dao.get_count(), 2)
        self.assertTrue(self.dao.get_latest_time() <= self.feed_info2[3])
        self.assertTrue(self.dao.get_latest_time() > self.feed_info2[3] - timedelta(days=1))
